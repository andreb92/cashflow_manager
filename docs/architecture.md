...
| `PaymentMethod` | Bank account, credit card, debit card, prepaid, cash |
...
`router.tsx` wraps all authenticated routes in `AuthGuard`. Unauthenticated requests to `GET /api/v1/me` return 401 → redirect to `/login`. After login, if `user_settings.onboarding_complete` is false → redirect to `/setup`.
...
- Recurring transaction instances share a `recurrence_group_id`; deleting the parent optionally cascades to all future instances
- Installment transactions share an `installment_group_id`; all N installments are created at once
